...
- **`examples/kagaz-showcase.html`** + **`kagaz-showcase.pdf`** — The canonical design system showcase: "Editorial precision, terminal warmth." Demonstrates the full palette, type specimens, schematic cards, glance grid, editorial list, data table, and invariants grid. **Primary calibration reference** — use it first when in doubt about how anything should look.
- **`examples/voyager-field-notes.html`** + **`voyager-field-notes.pdf`** — A long-form editorial document (Voyager probes). Shows multi-section layout: hero, glance grid, inline-SVG line chart, editorial dash-list, comparison table, spec grid, and blockquote. Good reference for data-heavy editorial pages.
- **`examples/slide-deck-example.html`** + **`voyager-deck.pdf`** — A working 6-slide 16:9 deck rendered to exactly 6 pages. Shows cover, metric-row, chart, bullet-list, and closing-quote slide layouts. Starting point for any slide deck; the PDF shows the finished print output.
