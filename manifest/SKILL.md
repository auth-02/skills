...
- **`data/`** holds source data and documents the task works with or produces as deliverables: PDFs, Excel (`*.xlsx`/`*.xls`), CSVs, TSVs, parquet, and JSON/dataset inputs. Create it on demand — the first time a PDF/spreadsheet/CSV-type file enters the task, put it here (preserve any meaningful grouping with subfolders, e.g. `data/results/`). Keep generated virtualenvs, `node_modules`, and caches **out** of the task tree entirely. The hub indexes `data/` documents (PDF/Excel/CSV/TSV), links them to their task as `task_has_data` lineage, and previews them inline (PDFs in-browser, spreadsheets/CSVs as tables) — so dropping a deliverable here makes it browsable and traceable from the task, another reason to use `data/` rather than scattering files at the task root.
